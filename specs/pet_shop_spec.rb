...
class TestPetShop < Minitest::Test
  ...
  def test_pet_shop_name
    name = pet_shop_name(@pet_shop) #passed in pet shop
    assert_equal("Camelot of Pets", name)   #it should come out "Camelot of pets"
  end
  ...
  def test_all_pets_by_breed__not_found
    pets = pets_by_breed(@pet_shop, "Dalmation")  # =... is the thing your defining
  ...
  def test_sell_pet_to_customer__pet_found
    customer = @customers[0]
    pet = find_pet_by_name(@pet_shop,"Arthur")        # retrieving previous method

    sell_pet_to_customer(@pet_shop, pet, customer)

    assert_equal(1, customer_pet_count(customer))     # Does the customer have a pet?
    assert_equal(1, pets_sold(@pet_shop))             # Has the shop sold the pet?
    assert_equal(100, customer_cash(customer))        # Has the customer paid for the pet?
    assert_equal(1900, total_cash(@pet_shop))         # Has the shop received the money?
  end

  def test_sell_pet_to_customer__pet_not_found
    customer = @customers[0]
    pet = find_pet_by_name(@pet_shop,"Dave")  # pet is nil

    sell_pet_to_customer(@pet_shop, pet, customer)  # pet is not a pet it is nil
